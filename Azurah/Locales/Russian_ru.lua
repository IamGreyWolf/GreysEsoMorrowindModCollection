...
L.ToggleCompassVisibility				= "Переключать компас видимость"
L.ToggleAbilityBlocking					= "Блокировка способности переключения"
L.AzurahAbilityBlock					= "Блокировка"
L.AzurahAbilityBlocked					= "|c67b1e9Azurah|r: Способность заблокирована настройками стержня."
...
L.SettingUseReadyColorTip				= "Установите, следует ли переключаться на альтернативный цвет, когда активная бар Ultimate готова к отмене. Изменяется на вышеупомянутый цвет, если не установлен, или когда недостаточно конечно."
...
L.GeneralAnchorDesc						= "Разблокируйте, чтобы разрешить перемещение Windows UI, используя мышь и изменено с помощью колеса прокрутки. Наложение будет отображаться для каждого разблокированного окна интерфейса пользовательского интерфейса, чтобы они могли быть перемещены, даже если они в настоящее время не отображаются (например, переместить целевое здоровье, если у вас нет цели).\n\nБлокировка сохранит любые изменения. Нажатие на «Отменить изменения» сбросит все изменения, поскольку Windows разблокированы. Щелкнув правой кнопкой мыши на одном окне сброс ожидающих изменений на это окно. 'Выход без сохранения' сохраняет ваши изменения без сохранения, чтобы вы могли видеть результат без двигателей, но он сбрасывается на следующий /reload, если вы не разблокируете снова и сохраните.\n\nВы можете ударить ESC, чтобы выйти из настроек Addon во время разблокировки или типа |cffff00/azurah unlock|r в чате, чтобы показать все кадры при перемещении. Используйте клавишу Toggle Cursor (по умолчанию '.'), Чтобы позволить вам переместить мыши на окна, чтобы переместить их."
L.GeneralEditFrameDesc					= "Эта опция позволяет вам модифицировать настройки для любого кадра, который вы переместили или изменяли измену с помощью Azurah.\n\nИзменение эффектов непрозрачности Как прозрачная рамка.\n\nВключение модификаций боевых боевых действий позволяет дополнительно устанавливать непрозрачность только в том случае, если вы находитесь в бою. На случай, если вы должны увидеть что-то скрытое."
L.GeneralUIOptions						= "Ui варианты кадров"
L.GeneralDescription1					= "для перемещения или масштабирования поддерживаемых кадров или"
...
L.GeneralDescription3					= "Вы должны |cffff00/reloadui|r после смены шкалы балансовой шкалы или шкала бара вызовут ошибки, которые могут ошибоваться. Также, если вы переключаетесь между клавиатурой и режимом GamePad, вы должны перезагрузить пользовательский интерфейс перед сменой кадров с Azurah."
L.GeneralEditFrames						= "Отредактируйте опции кадра UI"
L.GeneralEditFrameChoice				= "Выберите кадр, чтобы редактировать."
L.GeneralEditFrameNone					= "Нет кадров для редактирования."
L.GeneralEditFrameReset					= "Сбросить кадр"
L.GeneralEditFrameResetTip				= "Очистите все модификации азуры в выбранную кадр. Полезно, если меняет конфликт с другим капитальным ремонтом пользовательского интерфейса.\n\nПримечание: вы должны |cffff00/reloadui|r, прежде чем сброс вступит в силу."
L.GeneralAnchorUnlock					= "Разблокировать рамы пользователей"
...
L.General_GlobalOpacityTip				= "Включение этого здесь, или с помощью Keybind, временно переопределит любые индивидуальные настройки непрозрачности кадров и сделать все видимые кадры. Отключение этой опции использует отдельные настройки непрозрачности кадров, если установлено."
...
L.ActionBarBlockMageLight				= "Блок мага"
L.ActionBarBlockMageLightTip			= "Предотвратить активацию мага света и морфы при включении. Также может установить связь с ключами для включения/выключения."
L.ActionBarBlockExpertHunter			= "Блок Эксперт Охотник"
L.ActionBarBlockExpertHunterTip			= "Предотвратить активацию Expert Hunter и морфов, когда они включены. Также может установить связь с ключами для включения/выключения."
...
L.ActionBarOverlayUltPercentCapTip		= "При отображении относительного процента не показывайте значения выше 100%, если вы получаете больше ульты, чем ваши текущие затраты способностей."
...
L.CompassEnabledTip						= "Показать или скрыть компас. Может также быть установлен в переключатель ключей."
L.CompassLabelScale						= "Масштаб метки компаса"
L.CompassLabelScaleTip					= "Установите размер текста метки целевой площадки компаса."
L.CompassLabelPosition					= "Положение метки компаса"
L.CompassLabelPositionTip				= "Измените вертикальное положение текста метки относительно компаса вверх или вниз."
...
L.CompassResetTip2						= "Примечание. Модификация компаса Azurah не мешает Harvest Map, и как 3D-контакты, а также 3D-контакты, а также флажок «Разрешенные ресурс» работают нормально с модифицированным, перемещенным и измененным компасом."
...
L.Thievery_CTheftSaferTip				= "Влючает предотвращение обворовывания контейнеров, если вы не в скрытности."
L.Thievery_PTheftSafer					= "Безопасные карманные кражи"
L.Thievery_PTheftSaferTip				= "Влючает предотвращение карманной кражи, если вы не в скрытности."
...
L.EditFrame_Select						= "Выберите кадр, чтобы редактировать:"
L.EditFrame_Opacity						= "Рамочная непрозрачность"
L.EditFrame_OpacityTip					= "Устанавливает уровень прозрачности этого кадра."
L.EditFrame_CombatOpt					= "Борьба с вариантами"
L.EditFrame_CombatOptTip				= "Настройте изменения в кадры, которые будут отображаться только во время боя."
L.EditFrame_OpacityC					= "Боевая непрозрачность"
L.EditFrame_OpacityCTip					= "Устанавливает уровень прозрачности этого кадра при бою."
